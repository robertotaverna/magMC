...
!
   ...
   thetaB = acos(BS(1))
   B13 = BmodS/1.D13
   dens=8.9D3*eta*A*Z**(-0.6)*B13**(1.2)
   epe=0.0288D0*sqrt(dens*Z/A)
   eci=0.0635D0*(Z/A)*B13
   ece=115.77D0*B13
   ec=eci+(epe**2)/ece
   ecfx=(epe**2)/ece
   ...
   temp = temperaturek
!
   CALL PROBTOT(prob,temp)   
   write(*,'(2X,A15,I2,1H,,I2,$)')   'Patch ', ip,jp
   write(*,*) ' '
   write(*,*) 'theta=',acos(Ctin)*180.D0/Pi,'temp(keV)=',temp
   write(*,*) 'thetaB=',thetaB*180.D0/Pi, 'log10(Bmod)=',log10(BmodS)
   write(*,*) '# of photons:',Jma,'prob=',prob
   DO j = 1, Jmax
!
...
1     FlagLoss = 0 ; Nscatt = 0
      ...
      CALL STAR_EMIS(prob,temp,Cth0,Phi0,energy,Flagr)
      eps=energy
      Flag = Flagr
!
